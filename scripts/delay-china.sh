...
tc qdisc add dev enp0s8 parent 1:10 handle 10: netem delay 100ms 5ms

# tc qdisc add dev enp0s8 parent 1:20 handle 20: sfq perturb 10
tc qdisc add dev enp0s8 parent 1:20 handle 20: netem delay 125ms 10ms
...
# destination ip 192.168.12.0/24, and make the class 1:10 process the
# packets that match.
tc filter add dev enp0s8 protocol ip parent 1: prio 1 u32 match ip dst 192.168.3.21/32 flowid 1:10
